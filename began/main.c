...
int main()
{
    ...
    n.bitmap3 = al_load_bitmap("../startrek/UI/list/bg.jpg");
    n.bitmap4=al_load_bitmap("../startrek/UI/load/bg.jpg");
    n.sample1=al_load_sample("../startrek/UI/music/music1.wav");
    n.sample2=al_load_sample("../startrek/UI/music/music2.wav");
    bool event_timer = false;
    int checkout = 0.3*FPS;
    int git = checkout;
    int fp;
    if((fp=open("../startrek/UI/setting/setting",O_RDONLY))==-1){
        if((fp=open("../startrek/UI/setting/setting_R",O_RDONLY))==-1){
            screenflag=false;
            musicflag=true;
            volume_num=50.0;
            number=1;
            fp=open("../startrek/UI/setting/setting_R",O_CREAT|O_WRONLY);
            write(fp,&screenflag,sizeof(bool));
            write(fp,&number,sizeof(int));
            write(fp,&musicflag,sizeof(bool));
            write(fp,&volume_num,sizeof(int));
            close(fp);
            fp=open("../startrek/UI/setting/setting",O_CREAT|O_WRONLY);
            write(fp,&screenflag,sizeof(bool));
            write(fp,&number,sizeof(int));
            write(fp,&musicflag,sizeof(bool));
            write(fp,&volume_num,sizeof(int));
            close(fp);
            fp=open("../startrek/UI/setting/setting",O_RDONLY);
        }
    }
