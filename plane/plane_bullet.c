...
bool Draw_plane_bullet(allegro m,int file_num)
{
    ...
    while(1){
        ...
        if(redraw && al_is_event_queue_empty(m.event_queue)){
            ...
            if(bullet_num >= bullet_space){
                if(n.live){
                    ...
                    sprintf(num,"../UI/%d/bullet_01/bullet_0.png",screen_width);
                    n.bull[n.bullet_num].img = al_load_bitmap(num);
                ...
                for(int i = 0;i < MAXSIZE;i++){
                    if(enemy_plane[i].live && enemy_plane[i].blood != 0){
                        ...
                        enemy_plane[i].bull[enemy_plane[i].bullet_num].img = al_load_bitmap(num);
            ...
            if(plane_rate >= plane_space){
                ...
                if(enemy_plane[plane_num].level == 1){
                    enemy_plane[plane_num].img = al_load_bitmap(num);
                }
            ...
            for(int i=0;i<MAXSIZE;i++){
                ...
                sprintf(num,"../UI/%d/bullet_01/bullet_%d.png",screen_width,n.bull[i].form/5);
            ...
            for(int i=0;i<MAXSIZE;i++){
                if(enemy_plane[i].blood==0 && enemy_plane[i].live){
                    sprintf(num,"../UI/%d/boom_01/boom_%d.png",screen_width,enemy_plane[i].form/10);
            ...
            if(n.blood<=0&&n.live){
                sprintf(num,"../UI/%d/boom_01/boom_%d.png",screen_width,n.form/10);
        ...
        //飞机移动
        //mouse
        if(ev.mouse.x>0.5*al_get_bitmap_width(n.img)&&ev.mouse.x<game_width-0.5*al_get_bitmap_width(n.img)&&
                ev.mouse.y>0.5*al_get_bitmap_height(n.img)&&ev.mouse.y<game_height-0.5*al_get_bitmap_height(n.img)){
            if(Distance(n.x1,n.y1,ev.mouse.x,ev.mouse.y)>n.size)al_set_mouse_xy(m.display,n.x1,n.y1);
            else{
                n.x1=ev.mouse.x;
                n.y1=ev.mouse.y;
            }
